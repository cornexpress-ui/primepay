...
async def start_command(update: Update, context: CallbackContext) -> None:
    ...
    # Send welcome message as text (avoiding image URL issues)
...
async def button_callback(update: Update, context: CallbackContext) -> None:
    ...
    if data == "back_to_menu":
        await query.message.edit_text(
            "Select a premium channel to view details and subscribe:",
            reply_markup=main_menu_keyboard()
        )
        return
    ...
    if data.startswith("channel_"):
        ...
        # Fixed channel info display - text only for reliability
        channel_info = format_channel_info(channel_id)
        logger.info(f"Showing channel info for {channel_id}: {channel['name']}")
        
        try:
            # Simply show channel details in text with subscription button
            await query.message.edit_text(
                channel_info,
                parse_mode=ParseMode.HTML,
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🔒 Subscribe", callback_data=f"subscribe_{channel_id}")],
                    [InlineKeyboardButton("🔙 Back to Channels", callback_data="back_to_menu")]
                ])
            )
        except Exception as e:
            logger.error(f"Error displaying channel info: {e}")
            # Fallback with minimal formatting
            await query.message.edit_text(
                f"📚 <b>{channel['name']}</b>\n\n"
                f"Price: ₹{channel['price']} for {channel['validity_days']} days\n\n"
    ...
    if data.startswith("pay_"):
        ...
        if method == "upi":
            ...
            # QR code as text instead of image for reliability
            await query.message.edit_text(
                f"💳 <b>Pay via UPI QR Code</b>\n\n"
                f"Amount: ₹{channel['price']}\n\n"
                "Please scan the QR code sent to your UPI app.\n\n"
                "UPI ID: <code>{PAYMENT_METHODS['upi_id']}</code>\n\n"
                "After completing the payment, please send a screenshot for verification.",
                parse_mode=ParseMode.HTML,
                reply_markup=payment_methods_keyboard(channel_id, payment_id)
            )
...
async def screenshot_handler(update: Update, context: CallbackContext) -> int:
    ...
    try:
        ...
        # Fallback to just text notification
        await context.bot.send_message(
            chat_id=ADMIN_ID,
            text=(
                f"💰 <b>New Payment Verification</b>\n\n"
                ...
                f"Amount: ₹{payment['amount']}\n"
                f"Payment Method: {payment['payment_method']}\n\n"
                f"Payment screenshot received but couldn't be forwarded. "
