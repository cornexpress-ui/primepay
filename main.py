...
def handle_video(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        has_prefs = user_prefs and 'watermark_settings' in user_prefs
...
def process_with_progress(update: Update, context: CallbackContext, process_func, args, progress_msg):
    ...
    try:
        ...
        if result[0]:
            ...
            log_processing(user_id, "watermark_process", "success")
            return True
        else:
            progress_msg.edit_text("Error processing video. Please try again.")
...
def button(update: Update, context: CallbackContext) -> None:
    ...
    if choice == "screenshot":
        ...
        user_prefs = get_user_preferences(user_id)
        if user_prefs and 'watermark_settings' in user_prefs:
...
def handle_text(update: Update, context: CallbackContext) -> None:
    ...
    if context.user_data['awaiting_input'] == 'watermark_text':
        ...
        if context.user_data.get('save_settings', False):
            settings = {
                "watermark_settings.text": watermark_text
            }
            save_user_preferences(user_id, settings)
...
def main() -> None:
    ...
    try:
        ...
        global DATABASE_CHANNEL_ID
        if db and not DATABASE_CHANNEL_ID:
